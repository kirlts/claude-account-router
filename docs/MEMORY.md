...
## [HEU-009] A narrower rule must not inherit the broader rule's authority

**Date:** 2026-08-19
**Origin:** A folder router matched a directory to a profile by path prefix, then, to cover git worktrees living elsewhere, by repository identity. Adding a narrower route for one subfolder of an already routed repository made every worktree of that repository follow the subfolder's profile, because the subfolder reports the same repository as its parent.
**Pattern:** When a matcher has a specific dimension and a broad one, an entry added for the specific dimension silently acquires the broad one too. The more precise the exception, the wider the damage, which is the opposite of what the author intended and of what a reader of the config expects.
**Lesson:** When a rule can match on two dimensions of different width, gate the wider dimension on a property only the wider case has. Here, repository wide matching was restricted to routes that are their repository's top level. Also worth noting: the checker that found this had copied the same assumption, so fixing the engine turned the checker red. A duplicated assumption is a duplicated bug even when the code looks different.
**Source:** [Confirmed by user - no external source]

---

## [HEU-010] An indicator generated once is a claim about the past

**Date:** 2026-08-19
**Origin:** A command wrote a colored title bar into a folder to show which account it used. Routing later changed for that folder; the color stayed, advertising the old account while sessions ran under the new one.
